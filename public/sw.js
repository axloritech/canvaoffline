...
self.addEventListener("activate", (e) => {
  e.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((k) => k !== VERSION && !k.startsWith("rc-ocr")).map((k) => caches.delete(k)))).then(() => self.clients.claim()));
});
self.addEventListener("fetch", (e) => {
  ...
  e.respondWith(caches.match(req).then((cached) => {
    ...
    return fetch(req).then((res) => {
      if (res.ok && (url.pathname.startsWith("/_next/") || /\.(woff2?|png|svg|jpg|webp|ico|json|webmanifest|gz|wasm)$/.test(url.pathname) || url.pathname.startsWith("/ocr/"))) {
